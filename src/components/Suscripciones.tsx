...
import { Plus, Edit2, Trash2, CreditCard, CheckCircle2, XCircle, X, Search, Filter, AlertCircle, Loader2 } from 'lucide-react';
          ...
          {['Entrenador', 'Club', 'Colegio'].map((tab) => (
            ...
            >
              ...
              <span className="ml-2 inline-flex items-center justify-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">
                {suscripciones.filter(s => s.tipo === tab).length}
              </span>
            </button>
          ))}
        </div>
                  ...
                  <tr key={sub.id} className="hover:bg-gray-50/50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-gray-900">{sub.entidad}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-gray-900 font-bold">{sub.monto}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">{sub.proximo_cobro}</td>
                    <td className="px-6 py-4">
...
function NewSubscriptionModal({ defaultType, onClose, onSave, initialData }: { defaultType: string, onClose: () => void, onSave: (sub: any) => void, initialData?: any }) {
  ...
  const handleSave = () => {
    ...
    onSave({ entidad, tipo, plan, monto: `$${monto}`, proximo_cobro: proximo_cobro || '-', estado });
  };
